...
#with adaptive threshold and asy API
                ...
                try:
                    ...
                    with self.lock:
                        ...
                        if 200 <= res.status_code < 300:
                            ...
                            print(f"[API ✓] User {request['user_id']} - {res.status_code} {res.text}")
                        else:
                            self.stats["failed"] += 1
                            print(f"[API ✗] User {request['user_id']} - {res.status_code}")
...
def load_face_database(face_dir: str, net: cv2.dnn.Net) -> Tuple[np.ndarray, List[int]]:
    #Load PKL database (no enhancement applied)
    ...
    for f in os.listdir(face_dir):
        if f.endswith(".pkl"):
            ...
            with open(os.path.join(face_dir, f), "rb") as p:
                faces = pickle.load(p)
            embs = [get_face_embedding(face, net) for face in faces 
...
def run_face_recognition():
    ...
    known_embs, known_ids = load_face_database(FACE_DIR, mf_net)
    print(f"[INFO] Loaded {len(known_ids)} identities")

    cap = cv2.VideoCapture(VIDEO_SOURCE)
    cv2.namedWindow("Face Recognition", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Face Recognition", 600, 600)
...
PIPELINE: RETINAFACE + MOBILEFACENET (ASYNC API)
---------------------------------------------------------
